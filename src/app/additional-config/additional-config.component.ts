...
export class AdditionalConfigComponent implements OnInit {
  ...
  constructor(private demoCheckoutService: DemoServiceService,
    ...
    {}
    methods= [{label: 'Card', value: 'card'},{label: 'Netbanking', value: 'netbanking'},{label: 'Cardless Emi', value: 'cardless_emi'},{label: 'UPI', value: 'upi'},{label: 'Wallet', value: 'wallet'},{label: 'Emi', value: 'emi'}];

  ngOnInit(): void {
    this.form.valueChanges.pipe(debounceTime(3000)).subscribe((y) => {
      let a=this.getConfJsob(y)
      let z = JSON.parse(JSON.stringify(this.demoCheckoutService.getconfigcheckoutConfig()))
  ...
  public getConfJsob(ob): object {
    let toReturnObj =  {};
     toReturnObj={prefill:{contact:ob.contact,email:ob.email,method:ob.method}}    
       return toReturnObj;
     }
